...
public class GCodeGenerator {
    ...
    public static GCodeGenerator singlePass(List<Stroke> theStrokes,
                                            ...
                                            float cutoffRight, float spoilDepth, float xoffset) {
        GCodeGenerator gcg = new GCodeGenerator();

        // metadata
        gcg.comment("generated by CNC Carving Factory");
        gcg.comment(String.format("expected stock dimensions: %1.4fx%1.4fx%1.4f%s with %1.2f%s offset",
                stockWidth, stockLength, stockDepth, stockUnit, xoffset, stockUnit));
        ...
        for (Stroke s : strokes) {
            ...
            for (Anchor point : s) {
                ...
                if (!cutting) {
                    ...
                    gcg.cmd(new G(0).X(point.x * ipp + xoffset).Y(stockLength - point.y * ipp));
                    ...
                    gcg.cmd(new G(1).X(point.x * ipp + xoffset).Y(stockLength - point.y * ipp).Z(z).F(t.getLatSpeed()));
                }
            ...
            // emit pull out
            if (last != null) {
                gcg.cmd(new G(0).X(last.x * ipp + xoffset).Y(stockLength - last.y * ipp), true);
                gcg.cmd(new G(0).Z(clearancePlane));
            }
        }
